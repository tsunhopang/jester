...
class InferenceResult:
    ...
    def from_sampler(
        ...
        if sampler_type == "flowmc":
            ...
            metadata.update(
                {
                    ...
                    "n_likelihood_evaluations": int(
                        ns_metadata["n_likelihood_evaluations"]
                    ),
            ...
            if (
                "logZ_anesthetic" in ns_metadata
                and "logZ_err_anesthetic" in ns_metadata
            ):
                metadata["logZ_anesthetic"] = float(ns_metadata["logZ_anesthetic"])
                metadata["logZ_err_anesthetic"] = float(
                    ns_metadata["logZ_err_anesthetic"]
                )
    ...
    def add_eos_from_transform(
        ...
        if n_eos_samples is None or n_eos_samples >= n_available:
            ...
            logger.info(
                f"Generating EOS for {n_eos_samples} of {n_available} posterior samples"
            )
            # Randomly select subset
            idx = np.random.choice(
                np.arange(n_available), size=n_eos_samples, replace=False
            )
        ...
        transformed_samples = jax.lax.map(
            my_forward, chosen_samples_jax, batch_size=batch_size
        )
        TOV_end = time.time()
        logger.info(
    ...
    def summary(self) -> str:
        ...
        lines.append("\nPosterior Samples:")
        lines.append(
            f"  Parameters: {len(param_keys)} ({', '.join(param_keys[:5])}{'...' if len(param_keys) > 5 else ''})"
        )
