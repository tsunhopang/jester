...
def save_nicer_samples(
    ...
    np.savez(
        filepath, radius=radius, mass=mass, metadata=metadata  # type: ignore[arg-type]
    )
...
def process_maryland_data(
    zenodo_dir: Path, output_dir: Path, ignore_cache: bool = False
):
    ...
    for filepath in maryland_files:
        ...
        if output_filepath.exists() and not ignore_cache:
            ...
            print("    (use --ignore-cache to re-extract)")
            processed_files.append(output_filepath)
            continue

        # Save
        saved_path = save_nicer_samples(
            output_dir, psr, group, hotspot, data_used, variant, radius, mass, metadata
        )
...
def explore_amsterdam_archives(zenodo_dir: Path):
    ...
    archives = {
        "J0030 Amsterdam (Riley et al. 2019)": {
            "path": zenodo_dir
            / "J0030/amsterdam/original/A_NICER_VIEW_OF_PSR_J0030p0451.tar.gz",
            "mr_files": [
                "ST_S/ST_S__M_R.txt",
                "ST_U/ST_U__M_R.txt",
                "CDT_U/CDT_U__M_R.txt",
                "ST_EST/ST_EST__M_R.txt",
                "ST_PST/ST_PST__M_R.txt",
            ],
        ...
        "J0030 Amsterdam (Vinciguerra et al. 2023)": {
            ...
            "mr_files": [
                "ST_U/*_outputs/run1*/",
                "ST_PST/*_outputs/run1*/",
                "ST_PDT/*_outputs/run1*/",
                "PDT_U/*_outputs/run1*/",
            ],
        ...
        "J0740 Amsterdam (Salmi et al. 2022)": {
            "path": zenodo_dir
            / "J0740/amsterdam/intermediate/psr_J0740+6620_with_NICER_background_estimates.tar.gz",
            "mr_files": [
                "STU/3C50/STU_NICER_3c50bkgsms_hr_df3X/STU_outputs/run1/",
                "STU/W21_SW/STU_NICER_old/",
                "STU/3C50_XMM/*_outputs/run1/",
            ],
            "format": "MultiNest output files, recommend: STU_NICER_3c50bkgsms_hr_df3X",
        },
        "J0740 Amsterdam (Salmi et al. recent)": {
            "path": zenodo_dir
            / "J0740/amsterdam/recent/mr_samples_and_contours.tar.gz",
            ...
            with tarfile.open(archive_path, "r:gz") as tar:
                ...
                for pattern in info["mr_files"]:
                    ...
                    matching = [
                        m
                        for m in members
                        if any(part in m.name for part in pattern.split("/"))
                    ]
                ...
                if found_files:
                    ...
                    mr_files = [
                        ...
                        for m in members
                        if "M_R" in m.name
                        or ("mass" in m.name.lower() and "radius" in m.name.lower())
                    ]
                    if mr_files:
...
def parse_riley2019_mr_file(filepath: Path) -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    model = (
        ...
        if filepath.parent.name != "amsterdam"
        else filepath.stem.replace("__M_R", "")
    )
...
def extract_amsterdam_data(
        ...
        with tarfile.open(riley2019_archive, "r:gz") as tar:
            for mr_file_path in riley2019_mr_files:
                try:
                    ...
                    output_filename = (
                        f"J00300451_amsterdam_{model}_NICER_only_Riley2019.npz"
                    )
                    output_filepath = output_dir / output_filename
                    ...
                    print(
                        f"    Samples: {len(radius):,}, Size: {output_filepath.stat().st_size / 1024:.1f} KB"
                    )
                    extracted_files.append(output_filepath)
    ...
    salmi_recent_archive = (
        zenodo_dir / "J0740/amsterdam/recent/mr_samples_and_contours.tar.gz"
    )
        ...
        with tarfile.open(salmi_recent_archive, "r:gz") as tar:
            for mr_file_path in salmi_recent_mr_files:
                try:
                    ...
                    # Check if already extracted
                    output_filename = (
                        f"J07406620_amsterdam_gamma_NICERXMM_{variant}_recent.npz"
                    )
                    output_filepath = output_dir / output_filename
                    ...
                    # Extract to temporary location
                    print("  Extracting equal-weight samples...")
                    member = tar.getmember(mr_file_path)
