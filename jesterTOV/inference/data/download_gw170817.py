...
try:
    ...
    raise ImportError(
        "bilby is required for this script. Please install bilby via 'pip install bilby'."
    )
...
def load_hdf5_posterior(hdf5_path):
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        if posterior_data is None:
            ...
            root_datasets = [
                key for key in f.keys() if isinstance(f[key], h5py.Dataset)
            ]
            if root_datasets:
...
def explore_parameters(posterior, header):
    ...
    for p in header:
        if p in posterior:
            print(
                f"  {p}: min={posterior[p].min():.3f}, max={posterior[p].max():.3f}, mean={posterior[p].mean():.3f}"
            )
...
def extract_and_save(
    ...
    if has_source_masses:
        ...
        if "mass_1_source" in header:
            ...
            raise ValueError(
                "Source frame masses found but cannot identify parameter names"
            )
        ...
        if "m1_detector_frame_Msun" in header:
            ...
            raise ValueError(
                "Detector frame masses found but cannot identify parameter names"
            )
        ...
        raise ValueError(
            "Cannot find mass parameters in either source or detector frame"
        )
    ...
    if "lambda_1" in header:
        ...
        print(
            "WARNING: Only tidal parameters lambda_tilde/delta_lambda found, not individual lambdas"
        )
    ...
    # Add metadata
    conversion_note = (
        "Source frame masses already provided"
        if has_source_masses
        else "Masses converted from detector frame to source frame using bilby"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download and process GW170817 posterior samples"
    )
    parser.add_argument(
        "--ignore-cache",
        action="store_true",
    ...
    for dataset_name, url in URLS.items():
        ...
        if url.endswith(".hdf5") or url.endswith(".h5"):
            ...
            if args.ignore_cache or not data_file.exists():
                ...
                print(
                    f"File already exists: {data_file} (use --ignore-cache to re-download)"
                )
            ...
            if posterior is None:
                ...
                with h5py.File(data_file, "r") as f:
                    posterior_datasets = [
                        key for key in f.keys() if key.endswith("_posterior")
                    ]
                ...
                for posterior_name in posterior_datasets:
                    ...
                    posterior, header = load_hdf5_posterior_by_name(
                        data_file, posterior_name
                    )
                    ...
                    if len(parts) >= 2:
                        ...
                        waveform_model = (
                            f"{waveform_base}_{parts[-1]}"  # "IMRPhenomPv2NRT_highSpin"
                        )
                    else:
                        spin_type = "unknown"
                    ...
                    if output_file and output_file.exists():
                        ...
                        print(
                            f"Saved arrays: {[k for k in loaded.keys() if k != 'metadata']}"
                        )
                        for key in [
                            "mass_1_source",
                            "mass_2_source",
                            ...
                            if key in loaded:
                                print(
                                    f"  {key}: shape={loaded[key].shape}, dtype={loaded[key].dtype}"
                                )
                                print(
                                    f"    mean={loaded[key].mean():.3f}, std={loaded[key].std():.3f}"
                                )
            ...
            # Extract and save
            waveform_model = (
                "IMRPhenomPv2NRT_highSpin" if "gwtc" in dataset_name else "Unknown"
            )
            ...
            output_file = extract_and_save(
                posterior, header, dataset_name, waveform_model, data_source, event_name
            )
