    ...
    with h5py.File(hdf5_path, 'r') as f:
        ...
        def print_item(name, obj, indent=0):
            ...
            if isinstance(obj, h5py.Group):
                ...
                if obj.dtype.names:
                    print(f"{prefix}   Fields: {obj.dtype.names[:5]}..." if len(obj.dtype.names) > 5 else f"{prefix}   Fields: {obj.dtype.names}")
...
def load_posterior_by_path(hdf5_path, posterior_path):
    ...
    with h5py.File(hdf5_path, 'r') as f:
        ...
        print(f"Loaded {len(header)} parameters, {len(next(iter(posterior.values())))} samples")
...
def explore_parameters(posterior, header):
    ...
    for p in sorted(header):
        if p in posterior:
            vals = posterior[p]
            print(f"  {p:<40} min={vals.min():.3e}, max={vals.max():.3e}, mean={vals.mean():.3e}")
...
def extract_and_save(posterior, header, dataset_name, waveform_model, data_source="LIGO-P2000026"):
    ...
    if 'lambda_1' in header:
        ...
        print(f"Available lambda-like parameters: {[p for p in header if 'lambda' in p.lower() or 'tidal' in p.lower()]}")
        return None
    ...
    metadata = {
        ...
        'notes': 'Source frame masses provided' if has_source_masses else 'Converted from detector frame',
    }
...
def main():
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description='Download and process GW190425 posterior samples')
    ...
    for post_path in posteriors:
        ...
        try:
            ...
            if len(parts) == 2:
                ...
                waveform_base = parts[0].split(':')[-1] if ':' in parts[0] else 'Unknown'
            ...
            # Extract and save
            output_file = extract_and_save(posterior, header, dataset_name, waveform_model)
            ...
            if output_file and output_file.exists():
                ...
                for key in ['mass_1_source', 'mass_2_source', 'lambda_1', 'lambda_2']:
                    if key in loaded:
                        print(f"  {key}: shape={loaded[key].shape}, mean={loaded[key].mean():.3f}")
