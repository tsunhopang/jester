...
def tov_ode(h, y, eos):
    ...
    dedp = e / p * eos.dloge_dlogp_from_pressure(p)
    ...
    C0 = A * (-(2) * (2 + 1) / (r * r)
              + 4. * jnp.pi * (e + p) * dedp
              + 4. * jnp.pi * (5. * e + 9. * p)) \
        - (2. * (m + 4. * jnp.pi * r * r * r * p) / (r * (r - 2. * m)))**2
...
def tov_solver(eos, pc):
    ...
    dedp_c = ec / pc * eos.dloge_dlogp_from_pressure(pc)
    dhdp_c = 1. / (ec + pc)
