...
class TestInferenceResultSaveLoad:
    ...
    def test_save_load_flowmc_basic(self, temp_dir):
        ...
        np.testing.assert_array_equal(loaded.posterior['log_prob'], posterior['log_prob'])
        assert loaded.histories is not None
        np.testing.assert_array_equal(loaded.histories['local_accs'], histories['local_accs'])
    ...
    def test_save_load_smc_basic(self, temp_dir):
        ...
        np.testing.assert_array_equal(sampler_data['weights'], posterior['_sampler_specific']['weights'])
        np.testing.assert_array_equal(sampler_data['ess'], posterior['_sampler_specific']['ess'])

        # Check histories
        assert loaded.histories is not None
        np.testing.assert_array_equal(loaded.histories['lmbda_history'], histories['lmbda_history'])
    ...
    def test_save_load_ns_aw_basic(self, temp_dir):
        ...
        # Check sampler-specific data
        sampler_data = loaded.posterior['_sampler_specific']
        np.testing.assert_array_equal(sampler_data['logL'], posterior['_sampler_specific']['logL'])
        np.testing.assert_array_equal(sampler_data['logL_birth'], posterior['_sampler_specific']['logL_birth'])
    ...
    def test_save_load_with_derived_eos(self, temp_dir):
        ...
        np.testing.assert_array_equal(loaded.posterior['masses_EOS'], posterior['masses_EOS'])
...
class TestInferenceResultAddDerivedEOS:
    ...
    def test_add_derived_eos(self):
        ...
        np.testing.assert_array_equal(
            result.posterior['masses_EOS'],
            np.array([[1.4, 1.6, 1.8], [1.5, 1.7, 1.9]])
        )
