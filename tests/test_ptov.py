...
import numpy as np
from jesterTOV import ptov, utils
...
class TestPostTOVODE:
    ...
    def test_ptov_ode_vs_gr(self, sample_eos_dict, sample_ptov_eos_dict):
        ...
        p = 3.71e-11; e = 4.13e-27; m = 2.07e3; r = 1.20e4
...
class TestPostTOVSolver:
    ...
    def test_ptov_solver_different_mg_params(self, sample_eos_dict):
        ...
        mg_param_sets = [
            {"lambda_BL": 0.0, "lambda_DY": 0.0, "lambda_HB": 1.0, 
             "gamma": 0.0, "alpha": 10.0, "beta": 0.3},  # GR case
            {"lambda_BL": 0.1, "lambda_DY": 0.0, "lambda_HB": 1.0,
             "gamma": 0.0, "alpha": 10.0, "beta": 0.3},  # Brans-Dicke
            {"lambda_BL": 0.0, "lambda_DY": 0.05, "lambda_HB": 1.0,
             "gamma": 0.0, "alpha": 10.0, "beta": 0.3},  # dCS
            {"lambda_BL": 0.0, "lambda_DY": 0.0, "lambda_HB": 1.1,
             "gamma": 0.0, "alpha": 10.0, "beta": 0.3},  # Horndeski
        ]
...
class TestPostTOVPhysicalConsistency:
    ...
    def test_ptov_modified_gravity_effects(self, sample_eos_dict):
        ...
        gr_eos.update({
            "lambda_BL": 0.0, "lambda_DY": 0.0, "lambda_HB": 1.0,
            "gamma": 0.0, "alpha": 10.0, "beta": 0.3
        })
        
        # Modified gravity case
        mg_eos = sample_eos_dict.copy()
        mg_eos.update({
            "lambda_BL": 0.2, "lambda_DY": 0.1, "lambda_HB": 1.1,
            "gamma": 0.05, "alpha": 10.0, "beta": 0.3
        })
...
@pytest.mark.parametrize("mg_param,value", [
    ("lambda_BL", 0.1),
    ("lambda_DY", 0.05), 
    ("lambda_HB", 1.1),
    ("gamma", 0.02),
